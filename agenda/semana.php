...
<form name='seleccion_fecha' method='' action='' onsubmit='cambia_fecha();return false'>
    ...
    <input type='text' id='semana' name='semana' size ='10'
        value='<?php echo $hoy; ?>' onchange='cambia_fecha()' />
...
for ($k = 1; $k <= 7; $k++) {
    ...
    $borde="";
    $linea = "<th width='10%'>".$dia[$k]."<br>".$dia_anyo[$k]."</th>";
    if ($k == $hoy_es) {
        $linea ="<th width='10%' class='hoy'>".$dia[$k]."<br>".$dia_anyo[$k]."</th>";
    }
    $cadena.= $linea;
}
$cadena.="</tr>";
/**
 * Recorremos los despachos
 * @var integer
 */
for ($j = 1; $j <= 36; $j++) {
    if (!$despacho->isOcupado($j)) {
        $muestra = ($j == 23) ? "Sala de Juntas" : "Despacho ".$j;
        $cadena.="<tr><th width='10%'>".$muestra."<br/></th>";
        for ($i = 1; $i <= 7; $i++) {
            $cadena.="
            <td valign='top'>
            <span class='mini_boton'
            onclick='formulario_despacho_semana(".$j.", ".$dia_anyo[$i].")'>
            &nbsp;+&nbsp;</span>".
                datos_ocupacion($j, $dia_anyo[$i])."<p/></td>";
        }
        $cadena.="</tr>";
    }
}
